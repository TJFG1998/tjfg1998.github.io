...
var content_data = {
  ...
  "giveaway": {
    "title": "GIVEAWAYS",
    "description": "Prémios exclusivos em destaque. Participa agora e garante a tua entrada.",
    "linkText": "Ver Giveaways",
    "link": "/giveaways",
    "items": [
...
function renderGiveaway(giveaway) {
  ...
  if (!giveaway || !giveaway.items || giveaway.items.length === 0) {
    section.style.display = 'none';
    return;
  }
  section.style.display = '';

  document.getElementById('giveaway-title').textContent = giveaway.title;
  document.getElementById('giveaway-desc').textContent = giveaway.description;
  const link = document.getElementById('giveaway-link');
  link.href = giveaway.link;
  link.textContent = giveaway.linkText + ' →';

  const track = document.getElementById('ticker-track');
  const items = giveaway.items.concat(giveaway.items);
  items.forEach(item => {
    const card = el('div', { class: 'ticker-item' });
    card.innerHTML = `
